...
# Fs: pow many points to sample
# N: number of peaks to find
def unevenfft_res(filename,Fs,N,dub,opt,output):
	data=hnread(filename,"state")
	longtit=data[:,-3]+data[:,-2]
	longtit_s=center_angle(longtit[0],0,360)
	longtit=longtit[1:len(longtit)]
	#print sorted(longtit)
	longtit_center=np.array([center_angle(i-longtit_s,0,360) for i in longtit])
	longtit_or=longtit_center
	
	r=[calc_r(data[0,1],data[0,2],data[0,-1])-calc_r(data[i+1,1],data[i+1,2],data[i+1,-1]) for i in range(len(longtit)-1)]
	z=[calc_z(data[0,1],data[0,2],data[0,3],data[0,5],data[0,-1],data[0,4])-calc_z(data[i+1,1],data[i+1,2],data[i+1,3],data[i+1,5],data[i+1,-1],data[i+1,4]) for i in range(len(longtit)-1)]
	r_or=r
	z_or=z
